package foundation.functions
{
	...
	public function processVersion( p:Package ):void
	{
		...
		if ( !(p.error) ) {
			...
			//	CONST.initHardwareConst();
				...
				if (SERVER.HARDWARE_VER.charAt(0) == "1") {
					CONST.RELEASE = CONST.RELEASE_BOTTOM;
				} else {
					CONST.RELEASE = CONST.RELEASE_TOP;
				}
				
				// Регистрация параметров ver_info нижней платы
				SERVER.BOTTOM_SOFTWARE = CONST.RELEASE_BOTTOM;
			...
			if (pass) {
				Warning.show( loc("sys_connected")+" "+DS.name+" (" + fullversion+")", Warning.TYPE_SUCCESS, Warning.STATUS_DEVICE );
				founder.load();
			} else {
				load();
			}
		...
		function load(onlyservice:Boolean=false):void
		{
			...
			popup.open();
			Warning.show( loc("sys_connected_unknown_device")+ " (" + fullversion +")", Warning.TYPE_SUCCESS, Warning.STATUS_DEVICE );
