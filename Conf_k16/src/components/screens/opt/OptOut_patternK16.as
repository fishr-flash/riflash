package components.screens.opt
{
	...
	public class OptOut_patternK16 extends OptionsBlock
	{
		...
		private var _pattern_4_wasSelected:Boolean=false;
		
		public function set pattern_4_wasSelected(value:Boolean):void
		{
			_pattern_4_wasSelected = value;
		}
		public function get pattern_4_wasSelected():Boolean 
		{
			return _pattern_4_wasSelected;
		}
		
		private var pageY:int = 60;
		private var overrideSize:Function;
		
		public function OptOut_patternK16(f:Function)
		{
			super();
			...
			FLAG_SAVABLE = false;
			createUIElement( new FSComboBox, 0, loc("ui_pattern_output_control"), callSwitchPattern, 2, ClientArrays.OUT_PATTERNS ) as FSComboBox;
			attuneElement(250,300, FSComboBox.F_COMBOBOX_NOTEDITABLE );
			getLastElement().setCellInfo("0");
			
			note = new SimpleTextField( loc("output_note1"), 0, COLOR.RED);
			addChild( note );
			note.x = globalX;
			note.y = globalY + 10;
			note.visible = false;
		...
		private function pattern_1():void	
		{
			...
			if( !screens[1] ) {
				...
				(screens[1] as Vector.<Object>).push( 
					createUIElement( new FormString, 1,loc("output_ind_part_state"),null,3)
					);
				attuneElement(210);
				globalY = pageY;
				globalX = 260;
		...
		private function pattern_3():void	
		{
			...
			if( !screens[3] ) {
				...
				FLAG_SAVABLE = true;
				createUIElement( new FSShadow, CMD.OUT_PERMANENTLY_ON, "", null, 1 );
				FLAG_SAVABLE = false;
			}
			
			getField( CMD.OUT_PERMANENTLY_ON, 1 ).setCellInfo( 1 );
			
			if ( NEED_SAVE )
				SavePerformer.remember( getStructure(), getField(CMD.OUT_PERMANENTLY_ON,1) );
		}
		private function pattern_4():void
		{
			...
			if( !screens[4] ) {
				...
				var header:Header = new Header( [{label:loc("ui_pattern_device_state"),xpos:10},
					{label:loc("alarm_run_command"), xpos:257, width:200},
					],
					{size:12, leading:0} );
				...
				header.y = globalY;
				globalY += 40;
				(screens[4] as Vector.<Object>).push( header );
				
				
				/** Команда RELAY_INDMES настройки индикации непереданных событий  ( 3 - выхода - 3 структуры )
					Параметр 1 - 0 - Шаблон не действует, 1 - Шаблон действует;
				...
				addui( new FSShadow, CMD.OUT_INDMES, "1",null,1);
				
				var l:Array = UTIL.getComboBoxList( [ [0x00,loc("g_nocmd")],
					[0x01,loc("g_switchon")],
					[0x02,loc("g_switchoff")],
					[0x06,loc("out_switchon_05hz")],
					[0x07,loc("out_switchon_1hz")],
					[0x08,loc("out_switchon_2hz")] ] );  
				
				(screens[4] as Vector.<Object>).push( addui( new FSComboBox, CMD.OUT_INDMES, loc("output_events_exist"), null, 2, l ) );
				attuneElement( 250, 300, FSComboBox.F_COMBOBOX_NOTEDITABLE );
				(screens[4] as Vector.<Object>).push( addui( new FSComboBox, CMD.OUT_INDMES, loc("rfd_all_events_transfered"), null, 3, l ) );
				attuneElement( 250, 300, FSComboBox.F_COMBOBOX_NOTEDITABLE );
				
				FLAG_SAVABLE = false;
			}
			
			var data:Array = OPERATOR.dataModel.getData( CMD.OUT_INDMES )[ getStructure()-1 ];
			
			getField( CMD.OUT_INDMES, 1 ).setCellInfo( 1 );
			
			getField( CMD.OUT_INDMES, 2 ).setCellInfo( String(data[1]) );
			getField( CMD.OUT_INDMES, 3 ).setCellInfo( String(data[2]) );
			
			if ( NEED_SAVE )
				SavePerformer.remember( getStructure(), getField(CMD.OUT_INDMES,1) );
		...
		private function after():void
		{
			...
			if (pattern_1_wasSelected && CURRENT_SELECTION != 1)
				RequestAssembler.getInstance().fireEvent( new Request( CMD.OUT_INDPART, null, getStructure(), [0,0,0] ));
			if (pattern_2_wasSelected && CURRENT_SELECTION != 2)
				RequestAssembler.getInstance().fireEvent( new Request( CMD.OUT_ALARM1, null, getStructure(), [0,0] ));
			if (pattern_3_wasSelected && CURRENT_SELECTION != 3)
				RequestAssembler.getInstance().fireEvent( new Request( CMD.OUT_PERMANENTLY_ON, null, getStructure(), [0] ));
			if (pattern_4_wasSelected && CURRENT_SELECTION != 4)
				RequestAssembler.getInstance().fireEvent( new Request( CMD.OUT_INDMES, null, getStructure(), [0,0,0] ));
			
			pattern_1_wasSelected = CURRENT_SELECTION == 1;
			pattern_2_wasSelected = CURRENT_SELECTION == 2;
			pattern_3_wasSelected = CURRENT_SELECTION == 3;
			pattern_4_wasSelected = CURRENT_SELECTION == 4;
		}	
