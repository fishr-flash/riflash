package components.screens.opt
{
	...
	public class OptOut_ctrl3 extends OptionsBlock
	{
		private var panel:LevelPanelOff;
		
		private var pic:Bitmap;
		
		public function OptOut_ctrl3()
		{
			super();
			
			operatingCMD = CMD.OUT_OFF_LEVEL;
			
			var header:Header = new Header( [{label:loc("output_config_control_limits"),align:"left",xpos:10,width:300}], {size:12} );
			addChild( header );
			
			panel = new LevelPanelOff;
			addChild( panel );
			panel.y = 80;
			panel.x = 10;
		...
		override public function putState(re:Array):void
		{
			if (re[0] == CMD.OUT_CTRL_STATE ) {
				panel.putWireResistance( re[1][4] == 1 ? 1 : re[1][0] );
			}
		}
	}
}