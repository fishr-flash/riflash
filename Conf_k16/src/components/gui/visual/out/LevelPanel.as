	...
	public class LevelPanel extends UIComponent
	{
		...
		public function put(arr:Array):void
		{
			...
			for( var key:String in aDistrib) {
				lastx = servant.calcAPCtoX(aDistrib[key]);
				setPosition( int(key), lastx );
				if(mostx<lastx)
					mostx = lastx;
			}
		...
		private function fireChange():void
		{
			...
			for(var key:String in fields ) {
				if( (fields[key] as IFormString).param == max ) {
					levels.push( servant.getMaxACP() );
					continue;
				}
				if( (fields[key] as IFormString).param == 1 ) {
					levels.push( servant.getMinACP() );
					continue;
				}
				var tr:LevelThreshold = thresholds[int(key)-1];
				if ( tr )
