...
//				VoyagerBot.add( getLastElement() );	// добавляет поле с адаптером в спец массив, для более удобного управления полями
			...
			createUIElement( new FormString, 0, "час.", null, 8 ).x = fshift+32;
			...
			attuneElement( 30, NaN, FormString.F_EDITABLE );
			createUIElement( new FormString, 0, "мин.", null, 9 ).x = fshift + 96;
			attuneElement( 30, NaN, FormString.F_NOTSELECTABLE );
			
			globalY += 15;
			cshift = shift;
			createUIElement( new FormString, 0, "Пн.", null, 1 ).x = shift - 3;
			createUIElement( new FormString, 0, "Вт.", null, 2 ).x = placeX() - 3;
			createUIElement( new FormString, 0, "Ср.", null, 3 ).x = placeX() - 3;
			createUIElement( new FormString, 0, "Чт.", null, 4 ).x = placeX() - 3;
			createUIElement( new FormString, 0, "Пт.", null, 5 ).x = placeX() - 3;
			createUIElement( new FormString, 0, "Сб.", null, 6 ).x = placeX() - 3;
			createUIElement( new FormString, 0, "Вс.", null, 7 ).x = placeX() - 3;
		}
