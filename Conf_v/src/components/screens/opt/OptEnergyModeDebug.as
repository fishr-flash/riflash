package components.screens.opt
{
	...
	import components.basement.ComponentRoot;
	...
	public class OptEnergyModeDebug extends OptionsBlock
	{
		...
		public function OptEnergyModeDebug(s:int)
		{
			...
			var nostring:String = loc("g_no").toLowerCase();
			
			switch(s) {
				case 11:
					createUIElement( new FSComboBox, CMD.VR_WORKMODE_SET, loc("vem_mode")+":", callModeLogic, 1, 
						[{label:loc("vem_identify_coord_by_condition"), data:0}, {label:loc("vem_identify_coord_constant"), data:1}] );
					attuneElement( fieldShift, 344, FSComboBox.F_COMBOBOX_NOTEDITABLE );
					
					list = [{label:nostring, data:0},{label:loc("vem_constant"), data:4},{label:loc("vem_reg_with_interval"), data:3} ];
					list2= [{label:nostring, data:0},{label:"vem_single", data:1},{label:loc("vem_single_after"), data:2} ];
					
					xplace = 400;
					xshift2Cells = 63;
					
					FLAG_VERTICAL_PLACEMENT = false;
					createUIElement( new FSComboBox, CMD.VR_WORKMODE_ENGINE_START, loc("vem_engine_start")+":", callFiledLogic, 1, list2 );
					...
					createUIElement( new FormString, 0, loc("time_hour_s"), null, CMD.VR_WORKMODE_ENGINE_START ).x = xplace+32;
					attuneElement( 30 );
					createUIElement( new FormString, CMD.VR_WORKMODE_ENGINE_START, "", null, 3, null, "0-9", 2  ).x = xplace + 63;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					FLAG_VERTICAL_PLACEMENT = true;
					createUIElement( new FormString, 0, loc("time_min_s"), null, CMD.VR_WORKMODE_ENGINE_START+1 ).x = xplace + 96;
					attuneElement( 30 );
					
					FLAG_VERTICAL_PLACEMENT = false;
					createUIElement( new FSComboBox, CMD.VR_WORKMODE_ENGINE_RUNS, loc("vem_engine_work")+":", callFiledLogic, 1, list );
					attuneElement( fieldShift, 150+xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
					
					createUIElement( new FormString, CMD.VR_WORKMODE_ENGINE_RUNS, "", null, 2, null, "0-9", 2 ).x = xplace;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					createUIElement( new FormString, 0, loc("time_hour_s"), null, CMD.VR_WORKMODE_ENGINE_RUNS ).x = xplace+32;
					attuneElement( 30 );
					createUIElement( new FormString, CMD.VR_WORKMODE_ENGINE_RUNS, "", null, 3, null, "0-9", 2  ).x = xplace + 63;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					FLAG_VERTICAL_PLACEMENT = true;
					createUIElement( new FormString, 0, loc("time_min_s"), null, CMD.VR_WORKMODE_ENGINE_RUNS+1 ).x = xplace + 96;
					attuneElement( 30 );
					
					FLAG_VERTICAL_PLACEMENT = false;
					createUIElement( new FSComboBox, CMD.VR_WORKMODE_ENGINE_STOP, loc("vem_engine_stop")+":", callFiledLogic, 1, list2 );
					attuneElement( fieldShift, 150+xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
					
					createUIElement( new FormString, CMD.VR_WORKMODE_ENGINE_STOP, "", null, 2, null, "0-9", 2 ).x = xplace;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					createUIElement( new FormString, 0, loc("time_hour_s"), null, CMD.VR_WORKMODE_ENGINE_STOP ).x = xplace+32;
					attuneElement( 30 );
					createUIElement( new FormString, CMD.VR_WORKMODE_ENGINE_STOP, "", null, 3, null, "0-9", 2  ).x = xplace + 63;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					FLAG_VERTICAL_PLACEMENT = true;
					createUIElement( new FormString, 0, loc("time_min_s"), null, CMD.VR_WORKMODE_ENGINE_STOP+1 ).x = xplace + 96;
					attuneElement( 30 );
					
					FLAG_VERTICAL_PLACEMENT = false;
					createUIElement( new FSComboBox, CMD.VR_WORKMODE_START, loc("vem_start_move")+":", callFiledLogic, 1, list2 );
					attuneElement( fieldShift, 150+xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
					
					createUIElement( new FormString, CMD.VR_WORKMODE_START, "", null, 2, null, "0-9", 2 ).x = xplace;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					createUIElement( new FormString, 0, loc("time_hour_s"), null, CMD.VR_WORKMODE_START ).x = xplace+32;
					attuneElement( 30 );
					createUIElement( new FormString, CMD.VR_WORKMODE_START, "", null, 3, null, "0-9", 2  ).x = xplace + 63;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					FLAG_VERTICAL_PLACEMENT = true;
					createUIElement( new FormString, 0, loc("time_min_s"), null, CMD.VR_WORKMODE_START+1 ).x = xplace + 96;
					attuneElement( 30 );
					
					
					FLAG_VERTICAL_PLACEMENT = false;
					createUIElement( new FSComboBox, CMD.VR_WORKMODE_MOVE, loc("vem_move")+":", callFiledLogic, 1, list );
					attuneElement( fieldShift, 150 + xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
					
					createUIElement( new FormString, CMD.VR_WORKMODE_MOVE, "", null, 2, null, "0-9", 2 ).x = xplace;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					createUIElement( new FormString, 0, loc("time_hour_s"), null, CMD.VR_WORKMODE_MOVE ).x = xplace+32;
					attuneElement( 30 );
					createUIElement( new FormString, CMD.VR_WORKMODE_MOVE, "", null, 3, null, "0-9", 2  ).x = xplace + 63;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					FLAG_VERTICAL_PLACEMENT = true;
					createUIElement( new FormString, 0, loc("time_min_s"), null, CMD.VR_WORKMODE_MOVE+1 ).x = xplace + 96;
					attuneElement( 30 );
					
					FLAG_VERTICAL_PLACEMENT = false;
					createUIElement( new FSComboBox, CMD.VR_WORKMODE_STOP, loc("vem_stop_move")+":", callFiledLogic, 1, list2 );
					attuneElement( fieldShift, 150 + xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
					
					createUIElement( new FormString, CMD.VR_WORKMODE_STOP, "", null, 2, null, "0-9", 2 ).x = xplace;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					createUIElement( new FormString, 0, loc("time_hour_s"), null, CMD.VR_WORKMODE_STOP ).x = xplace+32;
					attuneElement( 30 );
					createUIElement( new FormString, CMD.VR_WORKMODE_STOP, "", null, 3, null, "0-9", 2  ).x = xplace + 63;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					FLAG_VERTICAL_PLACEMENT = true;
					createUIElement( new FormString, 0, loc("time_min_s"), null, CMD.VR_WORKMODE_STOP+1 ).x = xplace + 96;
					attuneElement( 30 );
					
					FLAG_VERTICAL_PLACEMENT = false;
					createUIElement( new FSComboBox, CMD.VR_WORKMODE_PARK, loc("vem_stop")+":", callFiledLogic, 1, list );
					attuneElement( fieldShift, 150 + xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
					
					createUIElement( new FormString, CMD.VR_WORKMODE_PARK, "", null, 2, null, "0-9", 2 ).x = xplace;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					createUIElement( new FormString, 0, loc("time_hour_s"), null, CMD.VR_WORKMODE_PARK ).x = xplace+32;
					attuneElement( 30 );
					createUIElement( new FormString, CMD.VR_WORKMODE_PARK, "", null, 3, null, "0-9", 2  ).x = xplace + 63;
					attuneElement( 30, NaN, FormString.F_EDITABLE );
					FLAG_VERTICAL_PLACEMENT = true;
					createUIElement( new FormString, 0, loc("time_min_s"), null, CMD.VR_WORKMODE_PARK+1 ).x = xplace + 96;
					attuneElement( 30 );
					
					list = [{label:nostring, data:0},{label:loc("vem_reg_with_interval"), data:3} ];
					
					FLAG_VERTICAL_PLACEMENT = false;
					createUIElement( new FSComboBox, CMD.VR_WORKMODE_REGULAR, loc("vem_regulary")+":", callFiledLogic, 1, list );
					...
					sched1 = new OptEnergySchedule(loc("vem_schedule")+" 1", getStructure(), fieldShift, xplace);
					addChild( sched1 );
					sched1.x = globalX;
					sched1.y = globalY;
					globalY += 33;
					
					sched2 = new OptEnergySchedule(loc("vem_schedule")+" 2", getStructure()+ 12, fieldShift, xplace);
					addChild( sched2 );
					sched2.x = globalX;
					sched2.y = globalY;
					globalY += 33;
					
					sched3 = new OptEnergySchedule(loc("vem_schedule")+" 3", getStructure() + 24, fieldShift, xplace);
					addChild( sched3 );
					sched3.x = globalX;
					sched3.y = globalY;
					globalY += 33;
					
					sched4 = new OptEnergySchedule(loc("vem_schedule")+" 4", getStructure() + 36, fieldShift, xplace);
					addChild( sched4 );
					sched4.x = globalX;
					sched4.y = globalY;
					globalY += 50;
				...
				case 12:
					createUIElement( new FSComboBox, CMD.VR_WORKMODE_SET, "", callModeLogic, 1, 
						[{label:loc("vem_go_online_by_condition"), data:0},{label:loc("vem_always_online"), data:1}] );
					attuneElement( NaN, 344, FSComboBox.F_COMBOBOX_NOTEDITABLE );
					getLastElement().x = fieldShift;
					
					list = [{label:nostring, data:0},{label:loc("vem_constant"), data:4},{label:loc("vem_reg_with_interval"), data:3} ];
					list2= [{label:nostring, data:0},{label:loc("vem_single"), data:1},{label:loc("vem_single_after"), data:2} ];
		...
		public function putAssemblege(a:Array, str:int):void
		{
			...
			if (a[CMD.VR_WORKMODE_SET][structureID-1][0] == 1) {
				...
				customDistribute( sched1 as ComponentRoot, a[CMD.VR_WORKMODE_SCHEDULE][structureID-1], CMD.VR_WORKMODE_SCHEDULE );
				customDistribute( sched2 as ComponentRoot, a[CMD.VR_WORKMODE_SCHEDULE][structureID-1+12], CMD.VR_WORKMODE_SCHEDULE );
				customDistribute( sched3 as ComponentRoot, a[CMD.VR_WORKMODE_SCHEDULE][structureID-1+24], CMD.VR_WORKMODE_SCHEDULE );
				customDistribute( sched4 as ComponentRoot, a[CMD.VR_WORKMODE_SCHEDULE][structureID-1+36], CMD.VR_WORKMODE_SCHEDULE );
		 ...
		 * @private
		 * 
		 *  Подменяет здесь вызов OptEnergyShedule.putRawData->ComponentRoot.distribute,
		 * чтобы обойти положенную проверку на поправку дня из за влияния UTC, и представить
		 * дни так как они записаны на приборе
		 * 
		 * @property component в данном случае одна из переменных sched+n
		 * @property arr массив данных для заполнения полей
		 * @property код команды
		 * 
		 */
		private function customDistribute( component:ComponentRoot, arr:Array, cmd:int ):void
		{
			var len:int = arr.length;
			for (var i:int=0; i<len; i++) 
			{
				component.getField( cmd, i + 1 ).setCellInfo( arr[ i ] );
			}
