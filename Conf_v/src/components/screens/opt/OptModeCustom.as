package components.screens.opt
{
	...
	public class OptModeCustom extends OptModeRoot
	{
		...
		public function OptModeCustom(s:int)
		{
			...
			var list:Array = [{label:loc("g_no"), data:0},{label:loc("vem_constant"), data:4},{label:loc("vem_reg_with_interval"), data:3} ];
			var list2:Array = [{label:loc("g_no"), data:0},{label:loc("vem_single"), data:1},{label:loc("vem_single_after"), data:2} ];
			var list3:Array;
			if (isEven)
				list3 = [{label:loc("vem_go_online_by_condition"), data:0},{label:loc("vem_always_online"), data:1}]
			else
				list3 = [{label:loc("vem_identify_coord_by_condition").toLowerCase(), data:0}, {label:loc("vem_identify_coord_constant").toLowerCase(), data:1}]
			
			createUIElement( new FSComboBox, CMD.VR_WORKMODE_SET, isEven?"":loc("vem_mode")+":", callModeLogic, 1, list3 );
			...
			if (VoyagerBot.isEngine()) {
				createUIElement( new FSComboBox, CMD.VR_WORKMODE_ENGINE_START, isEven?"":loc("vem_engine_start")+":", callFiledLogic, 1, list2 );
				...
				FLAG_VERTICAL_PLACEMENT = false;
				createUIElement( new FSComboBox, CMD.VR_WORKMODE_ENGINE_RUNS, isEven?"":loc("vem_engine_work")+":", callFiledLogic, 1, list );
				attuneElement( isEven?NaN:fieldShift, 150+xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
				if(isEven)
					getLastElement().x = fieldShift;
				...
				FLAG_VERTICAL_PLACEMENT = false;
				createUIElement( new FSComboBox, CMD.VR_WORKMODE_ENGINE_STOP, isEven?"":loc("vem_engine_stop")+":", callFiledLogic, 1, list2 );
				attuneElement( isEven?NaN:fieldShift, 150+xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
				if(isEven)
					getLastElement().x = fieldShift;
				...
				createUIElement( new FSShadow, 0, loc("time_hour_s"), null, CMD.VR_WORKMODE_ENGINE_START );
				createUIElement( new FSShadow, 0, loc("time_min_s"), null, CMD.VR_WORKMODE_ENGINE_START+1 );
				...
				createUIElement( new FSShadow, 0, loc("time_hour_s"), null, CMD.VR_WORKMODE_ENGINE_RUNS );
				createUIElement( new FSShadow, 0, loc("time_min_s"), null, CMD.VR_WORKMODE_ENGINE_RUNS+1 );
				...
				createUIElement( new FSShadow, 0, loc("time_hour_s"), null, CMD.VR_WORKMODE_ENGINE_STOP );
				createUIElement( new FSShadow, 0, loc("time_min_s"), null, CMD.VR_WORKMODE_ENGINE_STOP+1 );
			}
		
			
			FLAG_VERTICAL_PLACEMENT = false;
			createUIElement( new FSComboBox, CMD.VR_WORKMODE_START, isEven?"":loc("vem_start_move")+":", callFiledLogic, 1, list2 );
			attuneElement( isEven?NaN:fieldShift, 150+xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
			if(isEven)
				getLastElement().x = fieldShift;
			...
			FLAG_VERTICAL_PLACEMENT = false;
			createUIElement( new FSComboBox, CMD.VR_WORKMODE_MOVE, isEven?"":loc("vem_move")+":", callFiledLogic, 1, list );
			attuneElement( isEven?NaN:fieldShift, 150 + xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
			if(isEven)
				getLastElement().x = fieldShift;
			...
			FLAG_VERTICAL_PLACEMENT = false;
			createUIElement( new FSComboBox, CMD.VR_WORKMODE_STOP, isEven?"":loc("vem_stop_move")+":", callFiledLogic, 1, list2 );
			attuneElement( isEven?NaN:fieldShift, 150 + xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
			if(isEven)
				getLastElement().x = fieldShift;
			...
			FLAG_VERTICAL_PLACEMENT = false;
			createUIElement( new FSComboBox, CMD.VR_WORKMODE_PARK, isEven?"":loc("vem_stop")+":", callFiledLogic, 1, list );
			attuneElement( isEven?NaN:fieldShift, 150 + xshift2Cells, FSComboBox.F_COMBOBOX_NOTEDITABLE );
			if(isEven)
				getLastElement().x = fieldShift;
			...
			list = [{label:loc("g_no"), data:0},{label:loc("vem_reg_with_interval"), data:3} ];
			
			FLAG_VERTICAL_PLACEMENT = false;
			createUIElement( new FSComboBox, CMD.VR_WORKMODE_REGULAR, isEven?"":loc("vem_regulary")+":", callFiledLogic, 1, list );
			...
			sched1 = new OptEnergySchedule(isEven?"":loc("vem_schedule")+" 1", getStructure(), fieldShift, xplace, a);
			addChild( sched1 );
			sched1.x = globalX;
			sched1.y = globalY;
			globalY += 33;
			...
			sched2 = new OptEnergySchedule(isEven?"":loc("vem_schedule")+" 2", getStructure()+ 12, fieldShift, xplace, a);
			addChild( sched2 );
			sched2.x = globalX;
			sched2.y = globalY;
			globalY += 33;
			...
			sched3 = new OptEnergySchedule(isEven?"":loc("vem_schedule")+" 3", getStructure() + 24, fieldShift, xplace, a);
			addChild( sched3 );
			sched3.x = globalX;
			sched3.y = globalY;
			globalY += 33;
			...
			sched4 = new OptEnergySchedule(isEven?"":loc("vem_schedule")+" 4", getStructure() + 36, fieldShift, xplace, a);
			addChild( sched4 );
			sched4.x = globalX;
			sched4.y = globalY;
			globalY += 50;
