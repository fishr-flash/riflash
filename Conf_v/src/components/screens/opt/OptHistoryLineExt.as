package components.screens.opt
{
	...
	public class OptHistoryLineExt extends OptionListBlock
	{
		...
		public function OptHistoryLineExt(s:int)
		{
			super();
			...
			globalX = 0;//VoyagerHistoryServant.PARAMS[4].w+5;
			var counter:int=1;
			var columnwidth:int;
			
			var params:Object = {
				"HIS_COLLAPSED_PARAMS":HistoryDataProvider.HIS_COLLAPSED_PARAMS
			}
			...
			for(var i:int=0;i<256; ++i) {
				if (HistoryDataProvider.HIS_COLLAPSED_PARAMS[i] > 0) {
					...
					if (MISC.DEBUG_HISTORY_DIGITAL_VIEW == 1 && !VoyagerHistoryServant.PARAMS[i].fw )
						columnwidth /= 2;
					if ( !HistoryDataProvider.isVisible(i) )
						columnwidth = 0;					
			...
			if (s == 0) {
				var bOpts:VisualButton = new VisualButton( GuiLib.cGear );
				bOpts.x = globalX+10;
				bOpts.y = 5;
				bOpts.setUp( "", onOpts );
			}
			globalX += 45;
			width = globalX;//globalX < 780 ? 780 : globalX;
		...
		private function rebuildOrder():void
		{
			var params:Object = {
				"HIS_COLLAPSED_PARAMS":HistoryDataProvider.HIS_COLLAPSED_PARAMS,
				"HIS_ADAPTED_COLLAPSED_PARAMS":HistoryDataProvider.HIS_ORDER_PARAMS
			}
				
				// в этом блоке какой то из обьектов null object
				
			globalX = 0;
			var f:FormString;
			for (var i:int=0; i<256; ++i) {
				if (HistoryDataProvider.HIS_ORDER_PARAMS[i] > 0) {
					f = getField(operatingCMD, HistoryDataProvider.HIS_ORDER_PARAMS[i] ) as FormString;
					f.x = globalX;
					globalX += f.width;
				}
			}
		}
		override public function selectVertical(xpos:int):void
		{
			if (getStructure() == 0 )
				trace();
			var len:int = aCells.length;
			for (var i:int=0; i<len; ++i) {
				if ( (aCells[i] as UIComponent).x <= xpos && 
					(aCells[i] as FormEmpty).getWidth() + (aCells[i] as UIComponent).x >= xpos ) {
					vselection.visible = true;
					drawVSelection((aCells[i] as UIComponent).x, (aCells[i] as FormEmpty).getWidth());
				}
			}
		}
		...
		private function onOpts():void
		{
			GUIEventDispatcher.getInstance().fireEvent( GUIEvents, GUIEvents.EVOKE_TOGLE );
		}
	}
}