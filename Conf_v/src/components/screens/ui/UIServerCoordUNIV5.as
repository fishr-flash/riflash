package components.screens.ui
{
	...
	/**
	 *
	 * Дубликат класса UIServerCoordUNI для Voyager-5 
	 * с отключенным блоком настройки ЕГТС
	 *  
	 * @author r.kasymov
	 * 
	 */	
	public class UIServerCoordUNIV5 extends UI_BaseComponent
	{
		...
		public function UIServerCoordUNIV5()
		{
			super();
			...
			cbRitm.visible = false;
			globalY -= 20;
			go.add( "noegts", cbRitm );
			
			FLAG_SAVABLE = false;
			
			addui( new FormString(), 0, loc( "ui_coords_send_in_protocol_ritm" ), null, 1 );
			attuneElement( 468,NaN, FormString.F_MULTYLINE  );
			...
			//srvs[3] = new OptConnectServer(4,"IP адрес резервного сервера ЕГТС\r", "Порт резервного сервера ЕГТС");
			//srvs[3].egtsObject = true;
			addChild( srvs[3] );
			srvs[3].x = globalX;
			srvs[3].y = globalY;
			go.add("egts",srvs[3]);
				...
				//	SavePerformer.LOADING = true;
					...
					for (var i:int=0; i<len; ++i) {
						if( ( !DS.isfam( DS.F_V ) || ( DS.isfam( DS.F_V ) && DS.release > 54 ) ) )
						{
							if( i == 2 )
							{
								/// собираем номер объекта из данных размазаных по 3 и 4 ой стурктуре
								compoundName = ( Number( p.getStructure(i+2)[ 0 ] )<<16 )  + Number( p.getStructure(i + 1)[ 0 ] );
					...
					/*cbRitm.visible = !(p.length < 4);
					cbEgts.visible = !(p.length < 4);*/
