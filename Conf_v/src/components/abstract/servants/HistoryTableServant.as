package components.abstract.servants
{
	...
	public class HistoryTableServant
	{
		...
		private static var inst:HistoryTableServant;
		public static function access():HistoryTableServant
		{
			return inst;
		}
		...
		public function generateHeader():Array
		{
			...
			for(var i:int=2;i<256; ++i) {
				if (HistoryDataProvider.HIS_COLLAPSED_PARAMS[i] > 0 && HistoryDataProvider.isVisible(i) ) {
					t = loc(VoyagerHistoryServant.PARAMS[i].title);
					headers.push( [t, calcWidth(t)] );
					fields.push( VoyagerHistoryServant.PARAMS[i].field );
				}
			}
			return headers;
		}
		public function getHeaderLabelsForExport():Array	// порядок поменян, в переведенном виде
		{
			// необходимо [2] переставлять на 0 индекс и сдвигать весь массив, из-за искуственного смешения параметров истории на клиенте 
			var h:Array = [];
			var len:int = headers.length;
			for (var i:int=0; i<len; i++) {
				h.push( headers[i][0] );
			}
			var s:String = h.splice(2,1);
			h.splice(0,0,s);
			return h;
		}
		...
		public function getHeaderLocale():Array	// порядок поменян, в виде ключей локалей
		{
			return fields;
		}
		public function getFields():Array
		{
			if (!fieldsraw) {
				...
				for(var i:int=1;i<256; ++i) {
					if (HistoryDataProvider.HIS_COLLAPSED_PARAMS[i] > 0 && HistoryDataProvider.isVisible(i) ) {
						fieldsraw.push( VoyagerHistoryServant.PARAMS[i].field );
					}
				}
			}
			return fieldsraw;	
		...
		public function getContent(a:Array):Array
		{
			...
			for( var key:String in a ) {
				if (a[key] is Array && a[key].length > 0)
					content.push( put(a[key],true) );
			}
				
			return content;
		}
		public function getLineContent(a:Array):Array
		{
			return put(a,true);
		}
		...
		public function put(data:Array, update:Boolean=false):Array
		{	// update - если запрос из вне, не надо пересчитывать ширину. true = запрос изнутри
	...
	public function getRowColor(rowIndex:int, sourceColor:uint):uint
	{
		...
		if (a && a[rowIndex]) {
			...
			if (row.length > 0) {	// если радок т.е. информации в табилце нет вообще
				//if( row[row.length-1] && row[row.length-1] is String && (row[row.length-1] as String).toLowerCase() == loc("g_yes").toLowerCase() ) {
				if( row[row.length-1] is String && (row[row.length-1] as String).toLowerCase() == loc("g_yes").toLowerCase() ) {
					if (UTIL.isEven(rowIndex) )
						c = 0xeeaaa4;
					else
						c = 0xffdad7;
				}
				if( !(row[row.length-1] is String) && (row[row.length-1].data as String).toLowerCase() == loc("g_yes").toLowerCase() ) {
					if (UTIL.isEven(rowIndex) )
						c = 0xeeaaa4;
					else
						c = 0xffdad7;
