package components.screens.ui
{
	...
	public class UISensorTemperatureWTS extends UI_BaseComponent
	{
		...
		public function UISensorTemperatureWTS(group:int=0)
		{
			super();
		...
		private function keyDown(ev:KeyboardEvent):void
		{
			if (lastTarget) {
				...
				switch(ev.keyCode) {
					case KEYS.UpArrow:
						if ( lastTarget.y - shift >= firstScreen.y ) {
							lastTarget.y -= shift;
							changed = true;
						} else if (lastTarget.y != firstScreen.y ) {
							lastTarget.y = firstScreen.y;
							changed = true;
						}
						break;
					case KEYS.DownArrow:
						if ( lastTarget.y < firstScreen.y + GRAPH_HEIGHT ) {
							lastTarget.y += shift;
							changed = true;
						} else if (lastTarget.y != firstScreen.y + GRAPH_HEIGHT) {
							lastTarget.y = firstScreen.y + GRAPH_HEIGHT;
							changed = true;
						}
						break;
				}
...
import components.gui.limits.VectorDrawScreenU;
import components.protocol.Package;
import components.screens.ui.UISensorTemperatureWTS;
import components.static.COLOR;
...
class VectorScreen extends UI_BaseComponent
{
	...
	public function VectorScreen(title:String, timeline:Boolean=true)
	{
		...
		var container:LimitVContainer = new LimitVContainer( titles, UISensorTemperatureWTS.GRAPH_WIDTH, UISensorTemperatureWTS.GRAPH_HEIGHT);
		addChild( container );
		container.y = globalY;
		...
		var gear:Object = {w:UISensorTemperatureWTS.GRAPH_WIDTH, h:UISensorTemperatureWTS.GRAPH_HEIGHT, max_period:300, signal_resolution:1};
		graphStep = UISensorTemperatureWTS.GRAPH_WIDTH/gear.max_period;
		...
		if (DS.isDevice(DS.V2) ||  DS.isDevice( DS.K16)) {
		...
		globalY += UISensorTemperatureWTS.GRAPH_HEIGHT + 1;
	...
	override public function open():void
	{
		...
		hruler.build(rename(false),UISensorTemperatureWTS.GRAPH_WIDTH);
		if (vScreenOuter)
			vScreenOuter.clear();
		vScreenCpu.clear();
	}
	override public function put(p:Package):void
	{
		var res:Number = comb(int(toSignedLitleEndian(p.getStructure(2))));
	...
	private function getYByAcpForPaint(rawacp:Number):Number
	{
		...
		var coef:Number = UISensorTemperatureWTS.GRAPH_HEIGHT/(LIMIT_MAX-LIMIT_MIN);
		var num:Number = UISensorTemperatureWTS.GRAPH_HEIGHT - ( acp-LIMIT_MIN )*coef;
		if (num < 0 )
			num = 0;
		if (num > UISensorTemperatureWTS.GRAPH_HEIGHT )
			num = UISensorTemperatureWTS.GRAPH_HEIGHT;
		return num;
	}
