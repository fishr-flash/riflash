package components.screens.ui
{
	...
	public class UIPartitions extends UI_BaseComponent
	{
		
		private var opts:Vector.<OptPartitionInf>;
		
		private var iconLoader:IconLoader;
		public function UIPartitions()
		{
			super();
		...
		{
			const headLabels:Array = 
			[
				{ label:loc( "ui_user_part" ), xpos:0, width:100 },
				{ label:loc( "description" ), xpos:100, width:130 },
				{ label:loc( "g_show" ), xpos:230, width:150 }
				
			];
			
			const header:Header = new Header( headLabels );
			const header2:Header = new Header( headLabels );
			...
			opts = new Vector.<OptPartitionInf>();
			var xx:int = globalX;
			var yy:int = globalY;
			var len:int = OPERATOR.getSchema( CMD.KBD_PARTITION_NAME ).StructCount;
			var lastY:int = globalY;
			for (var i:int=0; i<len; i++) 
			{
				opts.push( new OptPartitionInf( i + 1 ) );
				opts[ i ].x = xx;
				opts[ i ].y = yy;
			...
			butReset.setUp( loc("sms_h_value_defaults" ), setDefault );
			butReset.y = globalY;
			butReset.x = globalX;
			...
			iconLoader.init();
			starterCMD = [ CMD.KBD_PARTITION_NAME ];
		...
		private function refine(value:Object):int
		{
			if(value is int) {
				...
				iconLoader.setAnyImageForLCD( 100 + value.struct, bitmap  );
		...
		private function setDefault():void
		{
			...
			for (var i:int=0; i<len; i++) 
			{
				opts[ i ].putRawData([ loc( "ui_user_part") + " ", ( i + 1 ) + ""   ] );
			}
...
class OptPartitionInf extends OptionsBlock
{
	...
	private function init():void
	{
		...
		multistring = addui( new FSSimple(), 0, loc( "ui_user_part" )+ " " + structureID, analiseText, 1, null, "", 21  ) as FSSimple;	
		attuneElement( 111, TextSnapshoter.WIDTH_TEXTFIELD, FSSimple.F_CELL_MULTYLINE );
		multistring.setHeight( TextSnapshoter.HEIGHT_TEXTFIELD );
		
		FLAG_SAVABLE = true;
		
		addui( new FSShadow(), CMD.KBD_PARTITION_NAME, "", null, 1, null, "", 10 );
		addui( new FSShadow(), CMD.KBD_PARTITION_NAME, "", null, 2, null, "", 10 );
		
		getLastElement().setCellInfo( loc( "ui_user_part" )+ " " + structureID );
		
		const box:FormEmpty = addui( new FSCheckBox(), CMD.KBD_PARTITION_NAME, "", null, 3 );
		attuneElement( 0, NaN );
		getLastElement().y = horLine;
		getLastElement().x = 256;
	...
	private function analiseText():void
	{
		...
		getField( CMD.KBD_PARTITION_NAME, 1 ).setCellInfo( multistring.cell.text.slice( 0, lineBreak >-1?lineBreak:int.MAX_VALUE ) );
		
		
		if( lineBreak > -1 )
		{
			getField( CMD.KBD_PARTITION_NAME, 2 ).setCellInfo( multistring.cell.text.split( "\r"  )[ 1 ]  );
		}
		
		SavePerformer.remember( structureID, getField( CMD.KBD_PARTITION_NAME, 1 ) );
	...
	override public function putRawData( a:Array ):void 
	{
		...
		getField( CMD.KBD_PARTITION_NAME, 1 ).setCellInfo( a[ 0 ] +  a[ 1 ] );
		getField( CMD.KBD_PARTITION_NAME, 2 ).setCellInfo( "" );
		getField( CMD.KBD_PARTITION_NAME, 3 ).setCellInfo( 1 );
		
		SavePerformer.remember( structureID, getField( CMD.KBD_PARTITION_NAME, 1 ) );
		SavePerformer.remember( structureID, getField( CMD.KBD_PARTITION_NAME, 2 ) );
	}
	
	override public function putData(p:Package):void 
	{
		pdistribute( p );
		const txt1:String = getField( CMD.KBD_PARTITION_NAME, 1 ).getCellInfo() as String;
		const txt2:String = "\r" + getField( CMD.KBD_PARTITION_NAME, 2 ).getCellInfo() as String;
