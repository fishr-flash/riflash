	...
	public class UIDevicePowerK14 extends UI_BaseComponent
	{
			...
			switch( p.cmd ) 
			{
				case CMD.BATTERY_LEVEL:
					if( p.getParam( 1 ) > 0 )
					{
						...
						if( _chart )
						{
							_chart.createHBarLine( "BL2"
								, COLOR.GREEN
								, 3 );
							
							if( _chart.lastBarY != rel )_chart.setBar( "BL2", rel, rel.toFixed( 2 ) + loc( "measure_volt_1l" ) );
						}
		...
		private function createChart( p:Package ):void
		{
			...
			if( !_chart )
			{
				...
				_chart.initField( minV, maxV, legend, new Rectangle( 0, 0, 710, 400 ), callDragRect, updateLimits );
		...
		private function callDragRect( name:String, oldRect:Rectangle ):Rectangle
		{
			var top:Number;
			var bottom:Number;
			var actual:Number = 0;
			var onv:Number = 0;
			var dragRect:Rectangle = oldRect;
			
			switch( name ) {
				case "3":
					top = _chart.getRealYOfLine( "5" );
					actual = _chart.getRealYOfLine( "4" );
					if( actual > top )  
							top = actual;
					
					top += _chart.getRealSize( Lp.GAP );
					
					bottom = _chart.getRealYOfLine( "2" ) ;
					actual = _chart.getRealPosition( Lp.MN_DSCH );
					if( actual < bottom )
							bottom = actual;
					
					bottom -= _chart.getRealSize( Lp.GAP );
					
					dragRect = new Rectangle( 0, top , 0, bottom - top );
					
					break;
				case "4":
					top = _chart.getRealYOfLine( "1" ) + _chart.getRealSize( Lp.GAP );
					actual = _chart.getRealPosition( Lp.MX_OND );
					
					if( actual > top ) top = actual;
					
					bottom = _chart.getRealYOfLine( "3" )  - _chart.getRealSize( Lp.GAP );
					actual = _chart.getRealSize( Lp.MX_OND );
					if( actual < bottom ) bottom = actual;
				...
				case "5":
					top = _chart.getRealYOfLine( "1" );
					top += _chart.getRealSize( Lp.GAP );
					
					actual = _chart.getRealPosition( Lp.MX_RCVR );
					...
					bottom = _chart.getRealYOfLine( "3" );
					actual = _chart.getRealYOfLine( "2" );
...
class Lp
{
	 ...
	 */
	public static var MN_DSCH:Number = 3.7;
	/**
