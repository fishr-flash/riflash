package components.screens.ui
{
	...
	public class UIVSensors extends UI_BaseComponent
	{
		...
		private var WATCH_ONBOARD_U:Boolean = false;
		
		private var container:LimitUContainer;
		private var bWatchU:TextButton;
		...
		public function UIVSensors()
		{
			...
			FLAG_SAVABLE = false;
			createUIElement( new FSSimple, 0, loc("sensor_limit_when_engine_working")
				, null, 2 );
			attuneElement( shift + 50, 50, FSSimple.F_CELL_NOTSELECTABLE );
			...
			bWatchU = new TextButton;
			bWatchU.setUp( loc("sensor_watch_onboard_voltage"), callLogic, 1 );
			addChild( bWatchU );
			bWatchU.x = globalX;
			bWatchU.y = globalY;
			...
			for (var i:int=0; i<2; ++i) {
				...
				register( hLimits[i] );
				hLimits[i].focusgroup = TabOperator.GROUP_LAST;
			}
			
			this.height = 590;
			
			starterCMD = [CMD.VR_VIBRO_SENSOR, CMD.VR_VOLTAGE_SENSOR];
		}
		...
		//	VoyagerBot.getInstance().askHistoryAKBstatus(doEnable);
		}
		...
		private function callLogic(num:int):void
		{
			...
			switch(num) {
				case 1:
					if (WATCH_ONBOARD_U) {
						deactivateSpamTimer();
						bWatchU.setName( loc("sensor_watch_onboard_voltage") );
					} else {
						initSpamTimer( CMD.VR_VOLTAGE_VALUE, 1, true, null, STATE_TIMER );
						bWatchU.setName( loc("sensor_disable_watching_voltage") );
					}
					WATCH_ONBOARD_U = !WATCH_ONBOARD_U;
					break;
				case 2:
