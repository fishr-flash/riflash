package components.screens.ui
{
	...
	{
		public static const WIFI_SECURITY_CB:Array = [[0,loc("g_no")],[2,"WPA/WPA2"]];
		public static const WIFI_SECURITY_SHOW:Array = [[0,loc("g_no")],[1,"WEP"],[2,"WPA/WPA2"]];	// чтобы правильно отображать WIFI_SECURITY_CB
...
//		открывать  добавление вайфая когда было именена страница а пототм она исчезла
		...
		private const STRING_GENERAL_CONFIG:String = loc("wifi_general_opts");
		...
		public function UIWifi()
		{
			...
			bAddDevice.setUp(loc("wifi_add"), changeNet, NET_ADD );
			...
			bRemoveDevice.setUp(loc("wifi_remove"), changeNet, NET_REMOVE );
			...
			bRestoreDevice.setUp(loc("g_cancel_remove"), changeNet, NET_RESOTRE );
			...
			tListEmpty = new SimpleTextField( loc("wifi_list_empty"), 170);
			tListEmpty.setSimpleFormat( "center", -7 );
			tListEmpty.y = -20;
			cont.addChild( tListEmpty );
			
			tNoDevices = new SimpleTextField( loc("wifi_no_nets"), 400);
			...
			//starterCMD = [CMD.SETTINGS_WIFI_NETS, CMD.GET_COUNT_WIFI_NETS];
			starterCMD = CMD.GET_COUNT_WIFI_NETS;
		}
				...
				case CMD.GET_COUNT_WIFI_NETS:
					if (p.getStructure(1)[1] == 1 && p.getStructure(2)[1] == 1) {
						if (p.getStructure()[0] == 0 && p.getStructure(2)[0] == 0) {
							...
							RequestAssembler.getInstance().fireReadSequence(CMD.WIFI_NETS_STORED, put, p.getStructure()[0]);
							RequestAssembler.getInstance().fireReadSequence(CMD.WIFI_NETS_VISIBLE, put, p.getStructure(2)[0]);
						}
					} else
						waitForState();
					break;
				case CMD.WIFI_NETS_STORED:
		...
		public function localResize(w:int, h:int, real:Boolean=false):void
		{
			...
			if (cont.y > navi.height + 75 )
				cont.y = navi.height + 75;
		}
		private function openNet(value:int, clicked:Boolean=true):void
		{
			...
			if (Controller.getInstance().isSaveActive() && !clicked) {
				...
				a = listwifi[navi.getButtonNameByIndex(value)];
				if (a) {
					lastSelected = value;
					opt.show( a, value+1 );
					lastSSID = opt.SSID;
		...
		private function changeNet(value:int):void
		{
			switch(value) {
				case NET_ADD:
					...
					break;
				case NET_REMOVE:
					RequestAssembler.getInstance().fireEvent( new Request(CMD.WIFI_NETS_CHANGE, null, 1, opt.getDeleteData() ));
		...
		private function refresh(p:Package=null):void 
		{
			if (this.visible) {
				if (p)
					...
					RequestAssembler.getInstance().fireEvent( new Request( CMD.GET_COUNT_WIFI_NETS, put));
				task.repeat();
			}
		}
		private function callState():void
		{	// вызывается когда сеть недоступна, запрашивается чаще чем раз в пол минуты
			RequestAssembler.getInstance().fireEvent( new Request(CMD.GET_COUNT_WIFI_NETS, onState) );
		}
