package components.screens.ui
{
	...
	public class UISysEvents extends UI_BaseComponent
	{
		...
		private var adapter:GMTAdapter;
		
		public function UISysEvents()
		{
			super();
			
			adapter = new GMTAdapter;
			structureID = 1;
			globalY = 40;
		//	globalX = 10;
			FLAG_SAVABLE = false;
			...
			if (DS.isfam( DS.K15 )) {
				addui( new FSShadow, CMD.SYS_NOTIF, "", null, 5 );
				addui( new FSShadow, CMD.SYS_NOTIF, "", null, 6 );
			} else {
				createUIElement( new FSCheckBox, CMD.SYS_NOTIF, loc("ui_sysev_gen_acu_fail"), null, 5 );
				attuneElement( 497 );
				getLastElement().focusgroup = 10;
			...
			for(var i:int=0; i<4; ++i) {
				if(i<3) {
					...
					opt.x = globalX;
					opt.y = 125;
					opt.setAdapter(new ZeroToNoneAdapter);
				}
				autoTest[i] = opt;
			}
			width = 530;
...
class GMTAdapter implements IDataAdapter
{
	public function change(value:Object):Object 	{ return value	}
	public function adapt(value:Object):Object
	{
		if ( String(value) == "255:255")
			return value;
		var utc:int = UTIL.mod( (new Date).timezoneOffset/60);
		var a:Array = String(value).split(":");
		if( int(a[0]) + utc > 23 )
			return UTIL.formateZerosInFront((int(a[0]) + utc - 24),2) + ":" + UTIL.formateZerosInFront(a[1],2);
		return UTIL.formateZerosInFront(int(a[0]) + utc,2) +":"+ UTIL.formateZerosInFront(a[1],2);
	}
...
class ZeroToNoneAdapter implements IDataAdapter
{
	public function adapt(value:Object):Object
	{
		return value;
