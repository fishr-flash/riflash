package components.screens.opt
{
	...
	{
		private var fsdelay:IFormString;
...
//			getLastElement().setAdapter( new StateAdapter );
			...
			if (DS.isfam( DS.K5 )) {
				...
				fsdelay = addui( new FormString, operatingCMD, "", null, 7, null, "0-9", 3, new RegExp(RegExpCollection.REF_0to255) );
		...
		private function onTrigger(t:IFormString=null):void
		{
			...
			if (isutil) {
				...
				if (b) {
					var savefield:IFormString;
					if (fsdelay is OptionsBlock)
						savefield = fsdelay.getFastField();
					else
						savefield = fsdelay;
					SavePerformer.remember( 1, savefield );
				}
