package components.screens.opt
{
	...
	public class OptRSim extends OptionsBlock
	{
		...
		public function OptRSim(s:int, onlyonesim:Boolean = false, fakesim:Boolean = true)
		{
			super();
			...
			_fApnAuto = addui( new FSCheckBox, CMD.GPRS_APN_AUTO, loc("ui_gprs_autoset_apn_setting"), onApn, 1 );
	...
	//		getField(CMD.K5_G_PHONE,1).setName(loc("ui_gprs_tel"));
			if (p.cmd == CMD.GPRS_APN_SELECT) {
				...
				else if (p.cmd == CMD.K5RT_GPRS_ADD)
				{
				}
				else
				{
	...
	//		getField(CMD.K5_G_PHONE,1).setName(loc("ui_gprs_current_sim_operator"));
		...
		private function callBlocker(value:Boolean):void
		{
			...
			fields[0].visible = !value;
			fields[1].visible = !value;
			fields[2].visible = !value;
			fields[3].visible = !value;
		...
		private function activateFields( upActiv:Boolean ):void
		{
			getField( CMD.K5_G_PHONE, 2 ).disabled = upActiv;
			getField( CMD.K5_G_APN, 2 ).disabled = upActiv;
			getField( CMD.K5_G_APN_LOG, 2 ).disabled = upActiv;
			getField( CMD.K5_G_APN_PASS, 2 ).disabled = upActiv;
		}
