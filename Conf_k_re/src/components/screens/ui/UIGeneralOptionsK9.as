package components.screens.ui
{
	...
	public class UIGeneralOptionsK9 extends UI_BaseComponent
	{
		...
		public function UIGeneralOptionsK9()
		{
			...
			twoSim = ( DS.isfam(DS.K9) != null );// && ( int(DEVICES.app) == 5 || int(DEVICES.app) == 2 );
			...
			if( DS.isDevice( DS.K9K ) || ( DS.isDevice( DS.K1M ) && int( DS.app ) === 7 ) )
			{
		...
		private function requestSignal():void
		{
			...
			if( DS.isDevice( DS.K9K ) )RequestAssembler.getInstance().fireEvent( new Request(CMD.BATTERY_LEVEL,put));
		...
		private function onSynch( i:int = 0 ):void
		{
			var d:Date = new Date;
