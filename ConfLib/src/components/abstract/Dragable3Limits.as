package components.abstract
{
	...
	public final class Dragable3Limits
	{
		public static const WIDHT_LIMITS_LINES:int = 600;
		public static const MAX_LEVEL_ON_DEVICE:int = 12600;
		public static var DIFFERENCE_BETWEEN:int = 100;
		...
		public function Dragable3Limits(c:UIComponent, flds:Array, p:Point, fto:Function, ffrom:Function, measure:String=null, dragable:Boolean=true)
		{
			ui = c;
			point = p;
			getTo = fto;
			...
			var GRAPH_WIDTH:int = WIDHT_LIMITS_LINES;
		...
		{	// изменяет баундинг бокс для перетаскивания
				hLimits[1].rect = new Rectangle(point.x,top,0,bottom-top);
				
				DIFFERENCE_BETWEEN = Math.round( ( getTo( 1000, true ) - getTo( ( getTo(  100 ) - DIFFERENCE_BETWEEN ), true  ) ) / 100 ) 
					
				
				
		}
		
		public function changeRectangleSeconds(top:Number, bottom:Number ):void
		{	// изменяет баундинг бокс для перетаскивания
			
			
			hLimits[0].rect = new Rectangle(point.x,top,0,bottom-top);	
			hLimits[2].rect = new Rectangle(point.x,top,0,bottom-top);	
		}
		...
		private function upData(save:Boolean=true, change:Boolean=true):void
		{
			...
			if( hLimits.length > 2 )
			{
				...
				updateRectangleLinesSecond();
			}

			if ( change) {
				for (var i:int=0; i<fields.length; ++i) {
					hLimits[i].color = colors[i];
					fields[i].setCellInfo(  ( getTo(hLimits[i].y)/100 ).toFixed( 1 ) );
					if (save)
		...
		private function updateRectangleLinesSecond():void
		{
			hLimits[2].rect = new Rectangle( hLimits[2].rect.x, hLimits[2].rect.top,0, ( hLimits[ 0 ].y - hLimits[2].rect.top )- DIFFERENCE_BETWEEN   );
			//hLimits[2].rect = new Rectangle( hLimits[2].rect.x, hLimits[ 0 ].y + DIFFERENCE_BETWEEN   ,0, hLimits[ 2 ].rect.bottom  - ( hLimits[ 0 ].y + DIFFERENCE_BETWEEN ) );
			hLimits[0].rect = new Rectangle( hLimits[0].rect.x, hLimits[ 2 ].y + DIFFERENCE_BETWEEN   ,0, hLimits[ 0 ].rect.bottom  - ( hLimits[ 2 ].y + DIFFERENCE_BETWEEN ) );
		}
	}
}