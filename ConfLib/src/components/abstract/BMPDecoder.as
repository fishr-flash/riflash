package components.abstract {
  ...
  public class BMPDecoder {
    ...
    private var _b16data:ByteArray;
	
	
	public function get b16data():ByteArray
	{
		return _b16data;
	}
     ...
     * Constructor
	 * 
	 * @author http://ntt.cc/2008/10/01/using-bmpdecoder-class-to-load-an-external-bmp-file-rle-compression-support.html
     */
	...
	/**
	 *  Извлекает и перекладывает бинарные данные, так
	 * чтобы short числа описывали следование пикселей
	 * изображения слева-направо и сверху-вниз
	 */
	private function flipofHor( ba:ByteArray ):void
	{
		ba.position = 0;
		const rowSize:int = nWidth;
		var rowCount:int = nHeight;
		_b16data = new ByteArray();
		_b16data.endian = Endian.LITTLE_ENDIAN;
		const sizeWord:int = 2;
		
		while( rowCount--)
		{
			
			ba.position = rowCount * sizeWord  * rowSize;
			
			for (var i:int=0; i<rowSize * sizeWord; i += sizeWord) 
			{
				_b16data.writeShort( ba.readShort() );	
			}
