package components.basement
{
	...
	public class UIRadioDeviceRoot extends UI_BaseComponent implements IResizeDependant
	{
		...
		protected var oldies:Vector.<int>;		// Сюда записываются СТРУКТУРЫ а не номера массива датчиков которые были найдены старыми на момент загрузки страницы
		...
		protected var nonRefundable:Boolean;
		...
		public function UIRadioDeviceRoot()
		{
			super();
		...
		{
			if( ev.keyCode == 90 && ev.ctrlKey && !bRestoreDevice.disabled )
				restoreDevice();
		}
...
//GUIEventDispatcher.getInstance().fireSystemEvent( SystemEvents.onBlockNavigation, {"isBlock":false} );
		...
		override public function put(p:Package):void
		{
			...
			for(var i:int; i<len; ++i ) {
				if ( isActive( p.data[i][0]) ) {
					...
					oDevices[i] = p.data[i];
					if( !treeContent )
						navi.addButton( getLabel(  i ) +" "+int(i+1), i, i*200 );
					else
						navi.addTree( getLabel( i ) +" " +int(i+1), i, treeContent );
					if (!opt.visible) {
		...
		/**
		 *  будет переопределена в UIKeyboard 
		 * для случаев когда надо различать типы клавиатуры
		 */
		protected function getLabel( str:int, device_type:int = 0 ):String
		{
			return label;
		}
...
//GUIEventDispatcher.getInstance().fireSystemEvent( SystemEvents.onBlockNavigation, {"isBlock":_value} );
		...
		protected function openDeviceSuccess( p:Package ):void
		{
			if ( !p.error ) {
				...
				changeSecondLabel( loc("navi_tuning")+" "+ labelParentPadejS+" "+ (currentActionDeviceId+1) + label_jumper );
			}
		}
				...
				switch( status ) {
					...
					case RF_STATE.SUCCESS:
						...
						if ( oDevices[struct-1] == null || ((REPLACE_RED || CLIENT.JUMPER_BLOCK)&& oDevices[struct-1][0] == 2) ) {
							...
							if( !treeContent )
								navi.addButton( getLabel(  struct, int( p.getParam( 1 ) ) ) +" "+struct, struct-1, (struct-1)*200 );
							else
								navi.addTree(  getLabel(  struct, int( p.getParam( 1 ) ) ) +" "+struct, struct-1, treeContent );
...
//	opt.putData( deletedDevice );
		...
		{		// должен передаваться массив с датой а не массив с массивами
		//	if (opt.getStructure() == p.structure )
			
			
				opt.putData(Package.create( p.data[0], p.structure ));
			...
			switch(action)
			{
				case RF_FUNCT.DO_CANCEL:
					RequestAssembler.getInstance().fireEvent( 
						new Request( CMD.RF_FUNCT, cancelSuccess, 1, [deviceType, currentActionDeviceId+1, RF_FUNCT.DO_CANCEL,0] ));
					break;
				case RF_FUNCT.DO_ADD:
					RequestAssembler.getInstance().fireEvent( 
						//new Request( CMD.RF_FUNCT, addDevice, 1, [ RF_FUNCT.TYPE_KEYBOARDS, currentActionDeviceId+1, RF_FUNCT.DO_ADD,0] ));
					new Request( CMD.RF_FUNCT, addDevice, 1, [deviceType, currentActionDeviceId+1, RF_FUNCT.DO_ADD,0] ));
					break;
				case RF_FUNCT.DO_DEL:
					RequestAssembler.getInstance().fireEvent( 
						new Request( CMD.RF_FUNCT, removeDevice, 1, [deviceType, navi.selection+1, RF_FUNCT.DO_DEL,0] ));
					break;
				case  RF_FUNCT.DO_RESTORE:
					RequestAssembler.getInstance().fireEvent( 
						new Request( CMD.RF_FUNCT, restoreDevice, 1, [deviceType, deletedDevice.structure, RF_FUNCT.DO_RESTORE,0] ));
					break;
			}
		}
