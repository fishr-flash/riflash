package components.protocol  
{
	...
	public class SocketProcessor extends Socket 
	{
		...
		public var timerIdleConnection:Timer;
		...
		{
			if ( instance == null )
				instance = new SocketProcessor( new Initiator );
			return instance;
		}
		
		public function SocketProcessor( _initiator:Initiator ) 
		{
			super();
			configureListeners();
			...
			timeout = 3000;
			timerIdleConnection = new Timer( CLIENT.TIMER_IDLE_INCOMPLETE, 1 );
		...
		public function sendManualRequest( _request:String, _func:Function ):void
		{
			...
			request.writeMultiByte( _request +"\r\n","windows-1251" );
		...
		private function closeHandler(event:Event):void
		{
			...
			if (SERVER.REMOTE_HOST == null || SERVER.REMOTE_HOST == 'localhost')
				performConnect();
		}
...
if (MISC.DEBUG_SHOW_SOCKETERRORS == 1) {
	dtrace( "securityErrorHandler() "+ event.text + getCon() );
		...
		private function timerDelay(delay:int):void
		{
			timerIdleConnection.repeatCount = delay/CLIENT.FIRST_DELAY;
		}
		private function timerStop():void
		{
			timerIdleConnection.stop();
		}
		private function timerRestart():void
		{
		...
		//		case PacketInspector.INCOMPLETE_B2:
				...
				case PacketInspector.GARBAGE:
					if(!inspector.MANUAL)	{
						...
						if( !DS.isfam( DS.K5RT3 ) )sendRequest();
					} else {
						inspector.MANUAL = false;
						response.clear();
					}
		...
		private function idleConnectionHappens( ev:TimerEvent ):void
		{
			...
			if( this.connected && !CLIENT.NOT_REQUEST_WHILE_IDLE) {
				Warning.show( DS.name+" (" +SERVER.VER_FULL + ") "+loc("sys_device_not_responding_retry") , Warning.TYPE_ERROR, Warning.STATUS_DEVICE );
				if(response) {
					if (incomplete) {
						var add:String="";
						if (!CLIENT.PROTOCOL_BINARY) {
				...
				if(fResponseProcessor is Function) {
					...
					dtrace("попытка реконнекта");
					
				} else
					timerStop();
				request = null;
			}
			timerDelay( CLIENT.DELAY_IDLE );
			dtrace("SocketProcessor.delay "+ timerIdleConnection.repeatCount*CLIENT.FIRST_DELAY);
