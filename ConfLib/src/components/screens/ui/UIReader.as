package components.screens.ui
{
	...
	public class UIReader extends UI_BaseComponent
	{
		...
		private var fieldCirc:IFormString;
		public function UIReader()
		{
			...
			var list:Array = [
								{label:loc("g_disabled_m").toLowerCase(), data:0},
								{label:loc("ui_reader"), data:1},
								{label:loc("ui_reader_external_control"), data:2}
							];
			...
			if( ( DS.alias != DS.K5 && !DS.isDevice(DS.K53G) ) || DS.release > 11 )
			{
			...
			else if( DS.release > 9 )
			{
				createUIElement( new FSShadow, CMD.READER_TM, "", null, 8 );
			}
			
			if( DS.isfam( DS.K5, DS.A_BRD ) )
			{
				globalY +=20;
				addui( new FSCheckBox, CMD.TM_KBD_ALARM_BEEP_ENABLE, loc("on_alarm_of_wirekeyb"), null, 1 );
				attuneElement( firstPadding + 135, NaN);
			}
			...
			FLAG_SAVABLE = false;
			fieldCirc = createUIElement( new FormString, 0, loc("ui_reader_part_guard_when_circuit"), null, 1 );
			attuneElement( firstPadding, NaN, FormString.F_MULTYLINE );
			globalY = anchor;
			FLAG_SAVABLE = true;
			...
			if( DS.isfam( DS.K5, DS.A_BRD ) )
				starterRefine( CMD.TM_KBD_ALARM_BEEP_ENABLE, true );
		...
		override public function put(p:Package):void
		{
			switch( p.cmd )
			{
				case CMD.READER_TM:
					...
					if( ( DS.alias != DS.K5 && !DS.isDevice(DS.K53G) ) || DS.release > 10 ) (getField( CMD.READER_TM, 8 ) as FSComboCheckBox).setList( PartitionServant.getPartitionCCBList( p.getStructure()[7] ));
		...
		private function callLogic(target:IFormString=null):void
		{
			...
			fieldCirc.disabled = management != INTERNAL;
			field.disabled = management != INTERNAL;
			if( management != INTERNAL )
